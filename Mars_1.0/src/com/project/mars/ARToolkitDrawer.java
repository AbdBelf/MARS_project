...
/**
 * ARToolKit Drawer ãƒžãƒ¼ã‚«ãƒ¼èª?è­˜éƒ¨åˆ†ã?ªã?©ã?Œå?«ã?¾ã‚Œã?Ÿã‚¯ãƒ©ã‚¹
 */
public class ARToolkitDrawer {
	...
	/**
	 * ä¸€å›žã?®å‡¦ç?†ã?§æ¤œå‡ºã?§ã??ã‚‹æœ€å¤§ã?®ãƒžãƒ¼ã‚«ãƒ¼æ•°ã€‚èª¤èª?è­˜åˆ†ã
	 * ‚‚å?«ã?¾ã‚Œã‚‹ã?®ã?§å°‘ã?ªã?™ã?Žã?¦ã‚‚é§„ç›®ã€?å¤šã?™ã?Žã‚‹ã?¨å‡¦ç?†è² è?
	 * ·å¢—
	 */
	 ...
	 */
	public ARToolkitDrawer(InputStream camePara, ArrayList<InputStream> patt,
			ModelRenderer mRenderer, boolean mTranslucentBackground,
			boolean isYuv420spPreviewFormat) {
	...
	/**
	 * åˆ?æœŸåŒ–å‡¦ç?†
	 * ãƒžãƒ¼ã‚«ãƒ¼ãƒ‘ã‚¿ãƒ¼ãƒ³ã€?ã‚«ãƒ¡ãƒ©ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿ã?¯ã?“ã
	 * ?“ã?§èª­ã?¿è¾¼ã‚€
	 */
	private void initialization(InputStream camePara,
	...
	/**
	 * NyARToolKitã?®åˆ?æœŸåŒ– ã‚«ãƒ¡ãƒ©ãƒ‘ãƒ©ãƒ¡ãƒ¼ã‚¿ã?®èª­ã?¿è¾¼ã?¿ Lire les
	 * paramÃ¨tres d'initialisation de la camÃ©ra
	 * 
	 * @param w
	 *            ã‚¹ã‚¯ãƒªãƒ¼ãƒ³ã‚µã‚¤ã‚º(width)
	 * @param h
	 *            ã‚¹ã‚¯ãƒªãƒ¼ãƒ³ã‚µã‚¤ã‚º(height)
	 */
	private void createNyARTool(int w, int h) {
		...
		try {
			if (nya == null) {
				...
				// ãƒžãƒ¼ã‚«ãƒ¼ã?®æž ç·šå¹…ã‚’å¤‰ã?ˆã‚‹ã?“ã?¨ã?¯å?¯èƒ½ã€‚
				// Possible de modifier la largeur de la bordure du marqueur.
				// NyARDetectMarker
				// å†…ã?«ã‚³ãƒ¡ãƒ³ãƒˆã‚’è¿½åŠ ã?—ã?Ÿã?®ã?§ã??ã?®éƒ¨åˆ†ã‚’å?‚ç…§ã?®ã?“ã?¨
				nya = new NyARDetectMarker(ar_param, ar_code, width, PATT_MAX,
	...
	/**
	 * æ??ç”»å‡¦ç?†éƒ¨åˆ† ãƒ¡ã‚¤ãƒ³ãƒ«ãƒ¼ãƒ—å‡¦ç?†ã?¨èª­ã?¿æ›¿ã?ˆã?¦ã‚‚è‰¯ã?„
	 * 
	 * @param data
	 */
	public void draw(byte[] data /* , int w , int h */) {
		...
		if (!isYuv420spPreviewFormat) {
			...
			Log.d("AR draw", "bitmap width * height()= " + width + " * "
					+ height);
			...
			Log.d("ARToolkitDrawer", "ARGB decode time: " + (time2 - time1)
					+ "ms");
		...
		if (!isYuv420spPreviewFormat) {
			...
			// !!!!!!!!!!!! Image convertis de YUV vers BitMap a fin de faire
			// les traitement dessu !!
			...
			long time2 = SystemClock.uptimeMillis();
			Log.d("ARToolkitDrawer", "RGB decode time: " + (time2 - time1)
					+ "ms");
		}
		...
		try {
			...
			/*
			 * ByteArrayOutputStream out = new ByteArrayOutputStream(); YuvImage
			 * yuvImage = new YuvImage(data, ImageFormat.NV21, width, height,
			 * null); yuvImage.compressToJpeg(new Rect(0, 0, width, height), 50,
			 * out); byte[] imageBytes = out.toByteArray(); Bitmap image =
			 * BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.length);
			 * 
			 * if (image != null) {
			 * 
			 * int otsu = OtsuTreshold(image); found_markers =
			 * nya.detectMarkerLite(raster, otsu); Log.d("Otsu ",
			 * "Otsu Treshold :" + otsu);
			 * 
			 * } else {
			 * 
			 * found_markers = nya.detectMarkerLite(raster, 128); Log.d("Otsu ",
			 * "Otsu Treshold None");
			 * 
			 * }
			 */
			...
			found_markers = nya.detectMarkerLite(raster, Constants.valeurBinarisation);
