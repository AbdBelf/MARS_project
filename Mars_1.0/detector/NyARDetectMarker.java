 ...
 * Il s'agit d'une recherche de classe AR par un code qui a été enregistré
 *  dans le constructeur, le code AR pour détecter les marqueurs multiples, 
 *  la meilleure correspondance pour chacun. Il reconnaît jusqu'à 300, 100 
 *  ou si c'est la limite je reconnais Gomiraberu.
 */
public class NyARDetectMarker
{
	...
	private class DetectSquareCB implements NyARSquareContourDetector.IDetectMarkerCallback
	{
		 ...
		 * Appelée chaque fois que le rectangle se trouve.
* On réserver les données de sommet pour examiner le motif de rectangles trouvés en tenant compte de l'orientation.
		 */
		public void onSquareDetect(NyARSquareContourDetector i_sender,int[] i_coordx,int[] i_coordy,int i_coor_num,int[] i_vertex_index) throws NyARException
		{
			...
			//輪郭座標から頂点リストに変換 
			//Vertex coordonnées dans une liste de contour
			...
			//Évaluer la différence de couleur est convertie en motif de l'acquisition de données.
			this._deviation_data.setRaster(this._inst_patt);

			//最も一致するパターンを割り当てる。
			//Assigner un modèle qui correspond le mieux.
			...
			//Enregistrée en tant qu'information de ce rectangle, le marqueur le plus d'informations cohérentes.
